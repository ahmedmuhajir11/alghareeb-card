...
import { Lock, User, Phone, Globe, Coins, X } from "lucide-react";

const WHATSAPP_GROUP_URL = "https://chat.whatsapp.com/CzNV13ovF3y8hZbHnYw2Je";
...
export default function ProfileSetupPage() {
  ...
  const [showWagModal, setShowWagModal] = useState(false);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (isFirstTime) {
        const wagKey = `wag_shown_${user?.id}`;
        if (!localStorage.getItem(wagKey)) {
          localStorage.setItem(wagKey, "1");
          setShowWagModal(true);
          return;
        }
      }
      setLocation("/");
  ...
  if (showWagModal) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4" dir="rtl">
        <div className="w-full max-w-sm bg-card border border-green-500/30 rounded-2xl p-6 shadow-2xl shadow-green-900/20 text-center relative">
          <button
            onClick={() => setLocation("/")}
            className="absolute top-3 left-3 text-muted-foreground hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <h2 className="text-xl font-black text-white mb-2">انضم لمجموعة متجرنا على واتساب!</h2>
          <p className="text-sm text-muted-foreground mb-1">
            نُبلّغ أعضاءنا فوراً بأي تغيير في الأسعار وأحدث العروض
          </p>
          <p className="text-xs text-muted-foreground mb-6">
            كن أول من يعرف عند رفع أو تخفيض الأسعار 🔔
          </p>
          <a
            href={WHATSAPP_GROUP_URL}
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => setTimeout(() => setLocation("/"), 500)}
            className="flex items-center justify-center gap-2 w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-xl transition-colors mb-3"
          >
