...
export function ServiceCard({ nameAr, nameEn, descriptionAr, imageUrl, icon, price, onRequest }: ServiceCardProps) {
  const { dir, isRtlLang, pick } = useDevLang();
  const displayName = isRtlLang ? nameAr : (nameEn || nameAr);
  return (
    <div
    ...
    >
      ...
      <div className="relative h-44 overflow-hidden">
        {imageUrl ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-primary/20 via-primary/10 to-transparent flex items-center justify-center">
            <span className="text-5xl">{icon || displayName.charAt(0)}</span>
          </div>
        )}
      ...
      <div className="p-4 space-y-3" dir={dir}>
        <div>
          ...
          {isRtlLang && nameEn && <p className="text-xs text-muted-foreground mt-0.5">{nameEn}</p>}
        </div>
        {descriptionAr && isRtlLang && (
        ...
        >
          {pick("اطلب الخدمة", "Request Service", "Hizmet İste")}
        </button>
      </div>
    </div>
  );
