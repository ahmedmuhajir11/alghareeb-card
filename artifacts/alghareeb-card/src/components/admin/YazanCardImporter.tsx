...
import { Loader2, Download, CheckSquare, Square, RefreshCw, ChevronDown, ChevronRight, AlertCircle, CheckCircle2, Package } from "lucide-react";
...
export default function YazanCardImporter() {
  const [products, setProducts] = useState<YzProduct[]>([]);
  const [categories, setCategories] = useState<Record<string, YzProduct[]>>({});
  ...
  async function fetchProducts() {
    ...
    try {
      const res = await fetch("/api/admin/yazancard/products", {
        credentials: "include",
      });
      if (!res.ok) {
  ...
  function toggleCatAll(cat: string) {
    const catProducts = (categories[cat] || []).filter(p => !filterAvail || p.available);
    const allSelected = catProducts.every(p => selected.has(p.id));
    setSelected(prev => {
      const s = new Set(prev);
      catProducts.forEach(p => {
        if (allSelected) s.delete(p.id);
        else s.add(p.id);
      });
      return s;
    });
  ...
  function selectAll() {
    const visible = products.filter(p => !filterAvail || p.available);
  ...
  async function doImport() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/yazancard/import", {
        method: "POST",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold flex items-center gap-2">
            ...
            استيراد منتجات يزن كارد
          </h2>
          <p className="text-muted-foreground text-sm mt-0.5">
            استورد المنتجات مباشرة من API يزن كارد إلى موقعك
          </p>
        </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-card/60 border border-border/40 rounded-xl">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                مثال: سعر شراء 10 → سعر بيع {(10 * (1 + markupPercent / 100)).toFixed(2)}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <p className="text-sm text-muted-foreground">
              {catNames.length} فئة — {totalAvailable} منتج متاح — <span className="text-primary font-semibold">{selected.size} محدد</span>
            </p>
            <Button
              onClick={doImport}
              disabled={importing || selected.size === 0 || !sectionId}
            ...
            {catNames.map(cat => {
              ...
              return (
                <div key={cat} className="border border-border/40 rounded-xl overflow-hidden">
                  <div
                    className="flex items-center justify-between p-3 bg-card/60 cursor-pointer hover:bg-card/80 transition-colors"
                    ...
                    <div className="divide-y divide-border/30">
                      {catProds.map(p => (
                        <div
                          ...
                          className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors text-sm ${selected.has(p.id) ? "bg-primary/10" : "hover:bg-card/60"}`}
                        >
      ...
      {!loading && products.length === 0 && !error && (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-sm">اضغط "جلب المنتجات" لبدء استيراد منتجات يزن كارد</p>
          <p className="text-xs mt-1 opacity-60">يتطلب تفعيل YAZANCARD_TOKEN في إعدادات الخادم</p>
        </div>
      )}
    </div>
  );
